...
import { createSkillAction, deleteSkillAction, updateSkillAction } from '@/app/admin/settings/skills/actions'

export function SkillsManager({ skills: initial }: { skills: Skill[] }) {
  const [isPending, startTransition] = useTransition()
  const [newSkill, setNewSkill] = useState({ name: '', level: 7, category: 'TECH' })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gray-900 rounded-2xl border border-gray-800 p-5">
        <h3 className="text-sm font-semibold text-gray-300 mb-4">Add New Skill</h3>
      ...
      <div className="bg-gray-900 rounded-2xl border border-gray-800 overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-800">
              <th className="text-left px-5 py-3 text-gray-400 font-medium">Skill</th>
              ...
              <tr key={skill.id} className="border-b border-gray-800/50">
                ...
                <td className="px-5 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-primary-400 text-xs font-mono">{skill.level}/10</span>
                  </div>
                </td>
        ...
        {!initial.length && (
          <div className="text-center py-10 text-gray-600">No skills added yet.</div>
        )}
      </div>
    </div>
  )
